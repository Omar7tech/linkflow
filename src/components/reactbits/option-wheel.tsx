...
const OptionWheel = ({
  ...
  // Upstream design: the rAF loop reads the latest props through refs, so it
  // never has to restart when a prop changes.
  ...
  // Named function expression so the loop can re-queue itself without
  // reaching for the outer binding before it is initialised.
  ...
  return (
    <div
      ...
      className={`relative h-full w-full overflow-hidden outline-none select-none [touch-action:none] ${
    ...
    >
      {items.map((label, index) => (
        <div
          ...
          className={`absolute top-1/2 cursor-pointer leading-none whitespace-nowrap will-change-[transform,opacity,filter] [font-size:var(--ow-font-size)] [color:color-mix(in_srgb,var(--ow-active-color)_calc(var(--ow-p,0)*100%),var(--ow-text-color))] ${
            side === "right"
