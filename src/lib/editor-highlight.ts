/**
 * Syntax highlighting for the live code editor.
 *
...
/** Token classes. Rendered as `ed-t{id}`; styled by the theme CSS variables. */
export const TT = {
  plain: 0,
  comment: 1,
  tag: 2,
...
/**
 * Paints one token type per character. Exported because the commands module
 * reuses it: knowing that a `{` is punctuation and not part of a string is
 * exactly what re-indenting and bracket matching need, and re-deriving that
 * with regexes would get it wrong on the first template literal.
 */
export function paintTypes(src: string, lang: EditorLang): Uint8Array {
  const types = new Uint8Array(src.length);
  if (src.length > MAX_HIGHLIGHT_CHARS) return types;
 ...
 *
 * Every source line becomes one row: a gutter cell plus the code cell. Keeping
 * the line number *inside* the row is what lets soft wrap work — a wrapped line
 * grows taller and its number stays pinned to its first visual row, which a
 * separate gutter column could never manage.
 */
export function highlightToHtml(src: string, lang: EditorLang): string {
  const lines = src.split("\n");
  const types = paintTypes(src, lang);
  ...
  for (let n = 0; n < lines.length; n++) {
    ...
    // The gutter cell is always emitted, even when numbers are switched off:
    // it is what holds the code column at the same x as the textarea's padding,
    // and CSS hides it without changing the layout.
    out.push(`<div class="ed-row"><span class="ed-ln">${n + 1}</span><span class="ed-code">`);
    ...
    if (line.length === 0) {
      ...
      for (let i = offset + 1; i <= end; i++) {
        ...
        if (t !== runType) {
          ...
          out.push(runType === TT.plain ? text : `<span class="ed-t${runType}">${text}</span>`);
          runStart = i;
          runType = t;
        }
      }
...
/**
 * Editor themes. Each one is a set of CSS variables, so switching is a single
 * attribute change on the wrapper — no re-tokenising, no repaint of the layer.
 */
...
export const EDITOR_THEME_IDS = new Set(EDITOR_THEMES.map((t) => t.id));

export function isDarkTheme(id: string): boolean {
  return EDITOR_THEMES.find((t) => t.id === id)?.dark ?? true;
}
